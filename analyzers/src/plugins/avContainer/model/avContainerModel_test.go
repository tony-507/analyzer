...
func readAdaptationFieldTest(t *testing.T) {
	...
	assert.Equal(t, expected, parsed, "Adaptation field not match")
}

func readPesHeaderTest(t *testing.T) {
	...
	expected := CreatePESHeader(234, 32165, CreateOptionalPESHeader(13, 705277, 694477))

	assert.Equal(t, expected, parsed, "PES header not match")
}

func TestReadSCTE35Section(t *testing.T) {
	...
	assert.Equal(t, expected, parsed, "SCTE-35 section not match")
}
